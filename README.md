...
3. 在环境变量编辑器中填写从 BotFather 获取的 Token：
...
Token 只保存在设备的 `/data/adb/canxin_oxidebot/env.conf`，不应提交到 GitHub、粘贴到
Issue 或写进 Rust 源码。
...
但不会创建 Release。推送与 `template.properties` 中版本一致的 tag，例如 `v0.1.0`，才会
构建签名 APK并发布正式 Release。

正式发布资产包括：

```text
canxin_oxidebot-v0.1.0.zip
canxin_oxidebot-manager-v0.1.0.apk
update.json
```
...
- [Telegram Bot OxideBot](https://github.com/canxin121/telegram_bot_oxidebot)
